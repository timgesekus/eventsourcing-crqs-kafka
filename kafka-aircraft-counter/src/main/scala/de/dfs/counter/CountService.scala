...
object CountService {
  ...
  def apply(config: Config, system: ActorSystem): ActorRef = {
    ...
    system.actorOf(Props(new CountService(consumerConf, actorConf)))
  }
}
...
class CountService(
  ...
  consumer ! Subscribe.AutoPartition(List(CountService.AC_COMMANDS_TOPIC), assignedListener, revokedListener)
  ...
  private def addFlight(ac: AC) = {
    val record = if (!acs.contains(ac.id)) {
      ...
      Some(KafkaProducerRecord(CountService.AC_EVENTS_TOPIC, ac.id.toString, message))
    } else {
  ...
  private def deleteFlight(id: Long) = {
    val record = if (acs.contains(id)) {
      ...
      Some(KafkaProducerRecord(CountService.AC_EVENTS_TOPIC, id.toString, message))
    } else {
      log.error("Flight does not exits")
      None
    }
  ...
  private def sendState(): Unit = {
    ...
    val stateRecord = KafkaProducerRecord(CountService.CONSUMER_STATE_TOPIC, "0", serializedState.toJson.compactPrint)
  ...
  private def commitReads(records: List[ConsumerRecord[String, String]]): Unit = {
    ...
    producer.sendOffsetsToTransaction(map, CountService.CONSUMER_GROUP_ID)
  ...
  private def reloadState: SerializedState = {
    val consumerConf = KafkaConsumer.Conf(
      new StringDeserializer,
      new StringDeserializer,
      groupId = CountService.CONSUMER_STATE_TOPIC,
      enableAutoCommit = false,
      autoOffsetReset = OffsetResetStrategy.EARLIEST,
      bootstrapServers = "localhost:9092")

    val consumer = KafkaConsumer(consumerConf)
    val topics = consumer.listTopics().get(CountService.CONSUMER_STATE_TOPIC)
